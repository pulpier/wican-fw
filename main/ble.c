...
/* --- WiCAN-OBD backport shims ------------------------------------------
 * This file is ported from the wican-pro branch. The following PRO-only
 * modules are not present in the classic WiCAN-OBD tree, so they are
 * replaced with no-op shims:
 *  - wifi_mgr:  not needed, WiFi stays up during BLE (coexistence patch)
 *  - cmdline:   BLE debug console, not ported
 *  - config_server_get_ble_power(): TX power GUI setting, PRO only --
 *    returning -1 keeps the compiled-in default TX power.
 */
static inline void cmdline_run_on_ble(const char *cmd) { (void)cmd; }
static inline void cmdline_print_prompt_on_ble(void) { }
static inline void cmdline_set_ble_output_func(void (*fn)(const char *data, size_t len)) { (void)fn; }
static inline int config_server_get_ble_power(int8_t *pwr_dbm) { (void)pwr_dbm; return -1; }
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
        ...
        case ESP_GATTS_CONNECT_EVT:
            ...
            // BLE/WiFi coexistence: keep WiFi and the config server running
            // while a BLE client is connected (see ble-wifi-coexist patch).
        ...
        case ESP_GATTS_DISCONNECT_EVT:
            ...
            // BLE/WiFi coexistence: WiFi was never stopped, nothing to restore.
            is_connected = false;
            ble_secured = false;
